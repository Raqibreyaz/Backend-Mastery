...
app.get(
  ...
  passport.authenticate("google", {
    failureRedirect:
      "http://localhost:3000/callback.html?error='Error Occured'",
    session: false, //tells we are not using express-session
  }),
  async (req, res) => {
    ...
    if (code) {
      ...
      return res.redirect(
        `http://localhost:3000/callback.html?sid=${session.id}`,
      );
    }
    return res.redirect(`http://localhost:3000/callback.html?error=${error}`);
  },
);
